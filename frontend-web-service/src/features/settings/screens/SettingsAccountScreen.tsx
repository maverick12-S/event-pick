...
import { FiArrowLeft, FiLock, FiMail, FiMapPin, FiTag, FiTrash2, FiUser } from 'react-icons/fi';
import PlanGuideModal from '../../accounts/components/PlanGuideModal';
import { ISSUE_SCREEN_SCALE, PLAN_OPTIONS } from '../../accounts/constants/accountsIssue.constants';
...
const SettingsAccountScreen: React.FC = () => {
  ...
  const detailQuery = useAccountDetailMock(CURRENT_ACCOUNT_ID);
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'アカウント情報を更新しました', severity: 'success' });
    } catch {
      setSnackbar({ open: true, message: 'アカウント情報の更新に失敗しました', severity: 'error' });
    }
  };
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await deleteMutation.mutateAsync(CURRENT_ACCOUNT_ID);
      setSnackbar({ open: true, message: 'アカウントを削除予定に設定しました（60日後に完全削除）', severity: 'success' });
    } catch {
      setSnackbar({ open: true, message: 'アカウント削除に失敗しました', severity: 'error' });
    }
  };

  const handleCancelDeletion = async () => {
    try {
      await cancelDeletionMutation.mutateAsync(CURRENT_ACCOUNT_ID);
    ...
    >
        ...
        >
          ...
          {!detailQuery.isLoading && !detailQuery.isError && form && detailQuery.data && (
            <Box sx={{ position: 'relative', zIndex: 1 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, mt: 0.3, mb: 1.25, flexWrap: 'wrap' }}>
                <ButtonBase
                  onClick={() => setConfirmPlanCancelOpen(true)}
                  sx={{
                    minHeight: 34,
                    px: 1.15,
                    borderRadius: '999px',
                    border: '1px solid rgba(255, 188, 119, 0.55)',
                    color: '#ffe9cf',
                    background: 'linear-gradient(145deg, rgba(207, 129, 54, 0.28), rgba(157, 82, 24, 0.26))',
                    fontSize: '0.8rem',
                    fontWeight: 800,
                  }}
                >
              ...
              <Typography sx={{ ...issueLabelSx, mt: 0.1 }}>アカウント状態</Typography>
              ...
              {form.status === '停止中' && (
                <Typography sx={{ mt: 0.7, color: 'rgba(255, 205, 186, 0.95)', fontSize: '0.82rem', fontWeight: 700 }}>
                  無効にして保存すると、停止状態になります。停止状態になるとアカウント削除が可能になります。
                </Typography>
              )}
              ...
              <Box sx={{ mt: 1.6, borderTop: '1px solid rgba(191, 214, 244, 0.24)', pt: 1.3 }}>
                ...
                <Typography sx={{ mt: 0.72, color: 'rgba(210, 227, 248, 0.72)', fontSize: '0.78rem', lineHeight: 1.6 }}>
                  削除は停止状態のアカウントのみ実行できます。まず「無効」を選択して保存してください。
                </Typography>
              </Box>
            </Box>
          )}
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            borderRadius: '12px',
            border: '1px solid rgba(223, 120, 141, 0.42)',
            background: 'linear-gradient(180deg, #fff8fa, #ffeef3)',
            boxShadow: '0 14px 34px rgba(45, 6, 18, 0.28)',
          },
        }}
      >
        <DialogTitle sx={{ color: '#77223a', fontWeight: 800, pb: 1 }}>アカウント削除確認</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: '#7a2d46', fontSize: '0.95rem', fontWeight: 600, lineHeight: 1.6 }}>
            このアカウントを削除予定に設定します。60日後に完全削除され、復元できません。
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 2.2, pb: 1.8, pt: 0.8, gap: 0.9 }}>
          <ButtonBase
            ...
            sx={{
              ...
              px: 1.6,
              borderRadius: '8px',
              border: '1px solid rgba(181, 121, 140, 0.62)',
          ...
          <ButtonBase
            ...
            sx={{
              ...
              px: 1.6,
              borderRadius: '8px',
              border: '1px solid rgba(207, 79, 114, 0.88)',
              background: 'linear-gradient(145deg, #e65f86, #c53e66)',
              color: '#ffffff',
              fontSize: '0.9rem',
              fontWeight: 800,
              '&:disabled': { opacity: 0.55 },
      ...
      <Dialog
        ...
        onClose={() => setConfirmPlanCancelOpen(false)}
        maxWidth="xs"
        fullWidth
        PaperProps={{
          sx: {
            borderRadius: '12px',
            border: '1px solid rgba(224, 171, 119, 0.5)',
            background: 'linear-gradient(180deg, #fffaf5, #fff3e7)',
            boxShadow: '0 14px 34px rgba(45, 25, 6, 0.22)',
          },
        }}
      >
        <DialogTitle sx={{ color: '#6c3f13', fontWeight: 800, pb: 1 }}>プラン解約の確認</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: '#7a542f', fontSize: '0.95rem', fontWeight: 600, lineHeight: 1.6 }}>
            プラン解約の手続きを進めますか？次の画面で解約内容を確認できます。
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 2.2, pb: 1.8, pt: 0.8, gap: 0.9 }}>
          <ButtonBase
            onClick={() => setConfirmPlanCancelOpen(false)}
            sx={{
              minHeight: 36,
              px: 1.6,
              borderRadius: '8px',
              border: '1px solid rgba(184, 145, 106, 0.7)',
          ...
          <ButtonBase
            ...
            sx={{
              ...
              px: 1.6,
              borderRadius: '8px',
              border: '1px solid rgba(214, 149, 73, 0.88)',
              background: 'linear-gradient(145deg, #f3b05d, #dc8f2d)',
              color: '#ffffff',
              fontSize: '0.9rem',
              fontWeight: 800,
            }}
